...
loop(exp_w,
  ...
  eta(alpha,exp_w) = sum(s,tau(s)*q_r.l(s))/(sum(s,tau(s)*(q_n.l(s)+q_r.l(s))));
  ...
  profit_r(alpha,exp_w) = sum(s, tau(s)*p.l(s)*q_r.l(s))                                      + (1-a)*p_rec.l*sum(s,tau(s)*q_r.l(s));
  profit_n(alpha,exp_w) = sum(s, tau(s)*p.l(s)*q_n.l(s) - 20*q_n.l(s) + 0.0005*power(q_n.l(s),2)) - a*p_rec.l*sum(s,tau(s)*q_r.l(s));
 );
);
